...
## 2026-06-06 — 标记精简 + 模拟试题解答

### 标记精简

移除了原有的 ★/▲ 标记，只保留老师考点标记（🔥/⭐），避免标记过多混淆。

### 模拟试题

同学根据课程总结制成了 8 题 100 分的模拟试卷：
- `Final/example.pdf`：原始试题 PDF
- `Final/样题解答.md`：完整详细解答（含推导步骤）

### 试卷覆盖与链接

| 题号 | 内容 | 分值 |
|------|------|------|
| Q1 | 概念问答（渐近展开/正则vs奇异/超越小量/Laplace vs 稳定相/WKB特征值） | 20 |
| Q2 | 非标准渐近展开 $\sqrt{25+x}$ → $\{1,\sin x,\cos x-1\}$ | 10 |
| Q3 | Laplace 方法 $\int e^{-x(\cosh t-1)}dt$ | 12 |
| Q4 | 边界层理论（精确解+MAE匹配+复合解） | 15 |
| Q5 | WKB + 转折点（Airy方程） | 12 |
| Q6 | 双时间尺度 Duffing（振幅方程） | 12 |
| Q7 | 鞍结分岔 $\dot{x}=\mu-x^2$ | 14 |
| Q8 | 离散化差分方程 | 5 |

### 解答校对

全部 8 题解答已对照试卷末尾的标准答案核对，数值与步骤一致。

**How to apply**：这套样题覆盖了老师强调的全部考试线路，可作为考前自测的最佳材料。

## 2026-06-09 — WKB 章节三处修正与补充

### 修正 1：§12.4（现 §12.5）WKB 通解多余因子

原式 $y = \frac{e^{S/\epsilon}}{Q^{1/4}}[\cdots]$ 多了一个 $e^{S/\epsilon}$ 因子。$1/Q^{1/4}$ 和两个指数项本身就是 $e^{S/\epsilon}$ 展开的结果（$S_0$ 产生指数，$S_1$ 产生 $Q^{-1/4}$），不需要再乘一次。已删除多余因子，并将等号改为 $\sim$。

### 补充 2：§12.3 解的形式——从 Liouville-Green 到 WKB 假设

原先直接从「假设 $y=e^{S/\epsilon}$」开始，跳过了两个关键步骤。新增完整的推导链：
1. **LG 变换动机**：$y''+Qy=0$ 通过 $ξ=∫√Q dx$、$Y=Q^{1/4}y$ 得到「指数 × 代数」结构
2. **先展开 S（按 σ）再 dominant balance**：设 $S=S_0+σS_1+\cdots$，代入得 $\frac{ε²}{σ²}(S_0')² + \frac{ε²}{σ}(\cdots) + \cdots = Q$。要按 ε 做摄动展开，首项系数 $ε²/σ²$ 必须 O(1) → σ = ε

**正确的逻辑顺序**：假设指数形式 → 展开 S（按 σ）→ 代入后看系数 → dominant balance 要求首项与 Q 同量级 → σ = ε → S 的展开自然变为按 ε。

### 补充 3：§12.7 转折点详细推导（原 §12.7+§12.8 合并）

原先只有寥寥几句，现在扩展为 9 个子步骤的完整求解流程：
1. WKB 在 Q(x₀)=0 失效的三个表现
2. 模板方程 $ε²y''=xy$
3. 局部线性化 Q≈a(x−x₀)
4. 引入内层伸缩 x−x₀=δZ（边界层逻辑）
5. Dominant Balance 定 δ=ε^{2/3}
6. 导出 Airy 方程 $Y_{ZZ}=ZY$，Ai/Bi 选择（物理约束）
7. Airy 渐近 ↔ WKB 外解匹配（两侧分别演示）
8. 标准单转折点连接公式
9. ASCII 流程图总结

核心逻辑链：WKB 失效 → 回到 boundary layer → dominant balance 定厚度 ε^{2/3} → rescaling → Airy → 匹配两侧 WKB → 连接公式。这与老师口述的考点完全对应。

**How to apply**：考试遇到 WKB 推导题时按 §12.3 的逻辑链写（LG 动机 → 设指数形式 → 展开 S → dominant balance → 逐阶收集）；转折点题按 §12.7.9 的流程图写。
